...
import { addToCart } from '../redux/breezySlice'

const ProductCards = ({product}) => {
  const dispatch = useDispatch()
  const navigate = useNavigate()
  const _id = product.title
  const rootId = _id.toLowerCase().split(' ').join('')
  const handleDetails = () => {
  ...
  return (
    <div className="group relative">
      ...
      <div className='w-full border-[1px] px-2 py-4'>
        ...
        <div>
          <h2 className='font-bold text-base font-titleFont'>{product.title.substring(0, 15)}</h2>
        </div>
        <div className='flex justify-end gap-2 relative overflow-hidden w-28 text-sm'>
         <div className='flex gap-2 group-hover:translate-x-24 transition-transform justify-end'>
